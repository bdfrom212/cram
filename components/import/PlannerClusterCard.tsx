...
export default function PlannerClusterCard({
  ...
  // Check if this cluster's name appears as an individual in another firm
  const knownParent = useMemo(() => {
    return personToFirm[displayName.toLowerCase().trim()] ?? null
  }, [personToFirm, displayName])

  // Find clusters with similar names (possible duplicates)
  const similarFirms = useMemo(() => {
    ...
    return allClusters.filter(c =>
      ...
      // Only show meaningful similarities (avoid single-word false matches)
      nameLower.length > 4 && c.proposed_name.toLowerCase().length > 4
    ).slice(0, 3)
  }, [allClusters, cluster])

  // Firms to show in the absorb search
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                title="This person works at another firm — link them there"
              >
              ...
              <button
                ...
                disabled={saving}
                className="flex-1 text-sm py-2 border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 disabled:opacity-50"
                title="This is a duplicate entry for the same firm"
              >
            ...
            <div className="flex gap-2">
              <button
                ...
                title="Two different firms got mixed into this one cluster"
              >
              ...
              <button
                ...
                title="Not a real firm or person you work with"
              >
                Skip — not relevant
              </button>
            </div>
