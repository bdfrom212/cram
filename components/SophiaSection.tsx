...
export default function SophiaSection({ eventId, initialBrief }: Props) {
  ...
  if (!brief) {
    return (
      <div className="border-t border-gray-100 px-5 py-4">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Content Draft</p>
        <button
          onClick={() => generate(false)}
          disabled={loading}
          className="w-full rounded-xl bg-rose-600 text-white px-4 py-3 text-sm font-medium hover:bg-rose-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? 'Sophia is drafting…' : 'Draft Instagram Content — Sophia'}
        </button>
  ...
  return (
    <div className="border-t border-gray-100 px-5 py-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Content Draft</p>
